      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-start justify-between mb-24 gap-12 border-l-2 border-accent pl-8">
          <div className="max-w-2xl">
            ...
            <h2 className="text-5xl md:text-7xl font-display italic mb-8">組織章程與運作透明</h2>
          ...
          <div className="grid grid-cols-2 gap-px bg-border border border-border font-mono">
            <div className="p-6 bg-surface text-center">
              <div className="text-xs text-muted mb-2 uppercase font-bold">Expeditions</div>
              <div className="text-3xl font-bold text-accent">450+</div>
            </div>
            <div className="p-6 bg-surface text-center">
              <div className="text-xs text-muted mb-2 uppercase font-bold">Transparency</div>
          ...
          {rules.map((rule) => (
            ...
            >
              <div className="flex items-baseline gap-6 mb-4 md:mb-0">
                <span className="font-mono text-xs text-accent font-bold">{rule.index}</span>
                <div>
                  <h3 className="text-xl font-display italic group-hover:pl-2 transition-all">{rule.title}</h3>
                  <p className="text-sm text-muted">{rule.desc}</p>
                </div>
              </div>
              <div className="flex items-center gap-8">
                ...
                <button className="font-mono text-[10px] uppercase tracking-widest hover:text-accent transition-colors font-bold">
                  View_PDF ↗
                </button>
              </div>
            </div>
