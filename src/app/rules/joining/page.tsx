...
export default function JoiningRulesPage() {
  const [data, setData] = useState<RuleCategory>(rulesData.joining);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    async function loadData() {
      try {
        const result = await ruleService.getRuleCategory("joining");
        setData(result);
      } catch (err) {
        console.error("Failed to load joining rules:", err);
      } finally {
        setIsLoading(false);
      }
    }
